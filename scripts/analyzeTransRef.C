...
string onm="Ploy_TransRef_PDGid2112_Ecut0.pdf";
string ofnm = "Ploy_TransRefl_PDGid2112_Ecut0.root";
const int nEnergy = 3;
...
void analyzeTransRef(){
  ...
  string materials[nMaterial] = {"Borated_Polythene"};
		/*	 "G4_W",
			 "G4_Pb",
			 "G4_Fe",
			 "G4_POLYETHYLENE",
  ...
  double thicknesses[nMaterial][nThickness]={{5,20,40,60,80,100,150,200}};
  //double thicknesses[nMaterial][nThickness]={{1,2,5,10,15,20}};
			/*    {5,7.5,10,12.5}};
...
void analyzeOneMaterial(string material, int pdgID, TGraph *g[2][2][nThickness+nEnergy], double thickness[nThickness], double effThick[nThickness]){
  ...
  for(int i=0;i<2;i++){
    ...
    for(int j=4;j<nThickness+nEnergy;j++){
...
void extractOneFile(int e, double thick, string mat, int pdgID,
  ...
  string fnm = Form("./Neutron%04dMeV-%03.1fcm%s/E_TranRefNeutron%04dMeV-%03.1fcm%s-1e5.root",
  ...
  t->Project("hT","prePosY:prePosX",Form("pType==%d && detID == 1 && %s",
					 pdgID,eCut.c_str()));
  hT->SetTitle(Form("%s Trans for %d %s| Ebeam=%d | thick = %f;x[cm];y[cm]",
  ...
  hR->SetTitle(Form("%s Refl for %d %s| Ebeam=%d | thick = %f;x[cm];y[cm]",
		    mat.c_str(),pdgID,eCut.c_str(),e,thick));
  hR->DrawCopy("colz");
  gPad->SetLogz(1);
