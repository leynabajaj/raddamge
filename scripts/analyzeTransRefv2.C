...
string onm="Mu1_y_TransRef_PDGid22_3cm_NotHP.pdf";
string ofnm = "Mu1_E_TranRef_PDGid22_3cm_NotHP.root";
const int nEnergy = 3;
const float nEv=1e5;
const int nMat = 1;
const int nThick = 4;
...
void analyzeTransRefv2(){
  ...
  double thickness[nMat][nThick]={{5,10,15,20}};
				 /* {1,5,10,15},
 ...
 /* double energy[21] = {25, 50, 100, 150, 200, 300, 400, 500,
  ...
  int energy[nEnergy] = {200,300,400};
...
void extractOneFile(int e, double thick, double effThick,
  ...
  for(int i=0;i<2;i++){
    string hTl=Form("%s | Ebeam=%d MeV| thick = %6.2f | TR = %s;kinetic energy [MeV]",
		    mat.c_str(),e,thick,tr[i].c_str());
    string hnm=Form("%s_%s_%d_%d",tr[i].c_str(),mat.c_str(),e,int(effThick));
